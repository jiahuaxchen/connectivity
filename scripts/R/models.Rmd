...
```{r}
block <- var_block %>% 
  dplyr::select(BLOCKID10,POP10,OVERALL_SC,sum_total_,sum_allroa,Point_Coun) %>% 
  drop_na(sum_total_) %>% 
  filter(sum_total_!=0) %>% 
  st_drop_geometry

model.block <- glm.nb(Point_Coun ~ OVERALL_SC + offset(log(sum_total_)), data = block)

summary(model.block)

cor.test(var_block$Point_Coun,var_block$OVERALL_SC)
```
