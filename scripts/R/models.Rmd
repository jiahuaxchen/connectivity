...
datasummary(cnnctvt + b_net_dens + b_gamma + b_cover + b_int_dens +
              ...
              Max + Histogram,data = connectivity_measures_df,output = 'result_tables/summary_table_v4.docx')
```
...
cor_sb <- var_df %>% 
  dplyr::select(phisp,incm_md,unmplyd,wht_prc,cnnctvt, b_net_dens,b_gamma,b_cover,b_int_dens,b_complex,ls_net_den,ls_gamma,ls_cover,ls_int_den,ls_complex) %>% 
  drop_na()
